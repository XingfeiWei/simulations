%% MCMC model 2D Hexaganol 10*10 network connection: H- V/ U\
%Three states model H(0 or +1, -1), V(0 or +1, -1), U(0 or +1, -1)
...
C=zeros(20,20,3);
    ...
    for i=1:l
        for j=1:b
            if mod(i,2)==0 && mod(j,2)==0 && mod(i,4)~=0
               C(i,j,:)=[1.0 0.0 0.0];
            elseif mod(i,4)==0 && mod(j,2)~=0  
               C(i,j,:)=[1.0 0.0 0.0];
                else
               C(i,j,:)=[1 1 1];
            end
...
xlim([0,41]);ylim([0,41]);
title(['AuNP 2D HEX array']); 
%%
nH1=1;nH2=1;nV1=1;nV2=1;nU1=1;nU2=1;
    l=20;
    b=20;
    for i=1:l
        for j=1:b
            if mod(i,2)==0 && mod(j,2)==0 && mod(i,4)~=0
               C(i,j,:)=[1.0 0.0 0.0]; %red NP
            elseif mod(i,4)==0 && mod(j,2)~=0  
               C(i,j,:)=[1.0 0.0 0.0]; %red NP
            elseif mod(i,2)==0 && mod(j,2)~=0 && mod(i,4)~=0
               C(i,j,:)=[0.5 0.5 1.0]; %blue H
            elseif mod(i,4)==0 && mod(j,2)==0  
               C(i,j,:)=[0.5 0.5 1.0]; %blue H
            elseif mod(i,2)~=0 && mod(i+1,4)~=0 && mod(j,2)~=0 
               C(i,j,:)=[1.0 1.0 0.1]; %yellow V
            elseif mod(i,2)~=0 && mod(i+1,4)==0 && mod(j,2)==0 
               C(i,j,:)=[1.0 1.0 0.1]; %yellow V
            elseif mod(i,2)~=0 && mod(i+1,4)~=0 && mod(j,2)==0 
...
xlim([0,41]);ylim([0,41]);
title(['AuNP 2D HEX array: nH=100, nV=100, nU=100']); 
%
ti=1;tj=1;tk=1;
nH1=0;nH2=0;nV1=0;nV2=0;nU1=0;nU2=0;p0=0.0;
    l=20;
    b=20;
    for i=1:l
        for j=1:b
            ...
            elseif mod(i,2)==0 && mod(j,2)~=0 && mod(i,4)~=0
               ...
               iniN(ti,1)=0;
                if rand(1)<p0
                if rand(1)<0.5
                C(i,j,:)=[0.75 0.75 1.0]; %blue H light
                nH1=nH1+1;
                iniN(ti,1)=1;
                else
                C(i,j,:)=[0.3 0.3 1.0]; %blue H dark
                nH2=nH2+1;
                iniN(ti,1)=-1;
                end
                end
            ...
            elseif mod(i,2)~=0 && mod(i+1,4)~=0 && mod(j,2)~=0 
               ...
               iniN(tj,2)=0;
                if rand(1)<p0
                if rand(1)<0.5
                C(i,j,:)=[1.0 1.0 0.75]; %yellow V light
                nV1=nV1+1;
                iniN(tj,2)=1;
                else
                C(i,j,:)=[1.0 0.9 0.01]; %yellow V dark
                nV2=nV2+1;
                iniN(tj,2)=-1;
                end
                end
            ...
            elseif mod(i,2)~=0 && mod(i+1,4)~=0 && mod(j,2)==0 
               ...
               iniN(tk,3)=0;
                if rand(1)<p0
                if rand(1)<0.5
                C(i,j,:)=[0.75 0.75 0.75]; %grey U light
                nU1=nU1+1;
                iniN(tk,3)=1;
                else
                C(i,j,:)=[0.5 0.5 0.5]; %grey U dark
                nU2=nU2+1;
                iniN(tk,3)=-1;
                end
                end
...
xlim([0,41]);ylim([0,41]);
title(['Random link p=',num2str(p0),': nH=',num2str(nH1+nH2),', nV=',num2str(nV1+nV2),', nU=',num2str(nU1+nU2)]); 
%% MC simulation 
% list H V U link ids for each NP 
% neighbor list 100H 100V 100U** first row: -> H1-H10 and first column: |^ V1-V91
NPx=zeros(100,2,3);
for i=1:100
    temp = floor((i-1)/10);
    if mod(temp,2)==0
    ...
    NPx(i,2,3)=i+10;
    if mod(i,10)==0
        NPx(i,2,1)=i+1-10;end
    end
    if mod(temp,2)==1 && temp~=9
    NPx(i,1,1)=i-1;NPx(i,1,2)=i-1;NPx(i,1,3)=i;
    NPx(i,2,1)=i; NPx(i,2,2)=i+10;NPx(i,2,3)=i+10-1;
    if mod(i,10)==1
        NPx(i,1,1)=NPx(i,1,1)+10;
        NPx(i,1,2)=NPx(i,1,2)+10;
        NPx(i,2,3)=NPx(i,2,3)+10;
    end
    end
    if temp==9
        NPx(i,1,1)=i-1;NPx(i,1,2)=i-1;NPx(i,1,3)=i;
    ...
    if mod(i,10)==1
        ...
        NPx(i,2,2)=NPx(i,2,2)-100;
        NPx(i,2,3)=NPx(i,2,3)-100;
    end
end
% build neighbour list for H(2h,4v,4u) V(4h,2v,4u) U(4h,4v,2u)
% Horizontal links
clear nbH nbV nbU 
for i=1:100 %for nbH
    k=0;
...
for i=1:100 % for nbU
    ...
    if NPx(j,2,3)==i
        ...
        for i2=1:2
                k=k+1;
                nbU(i,k)= NPx(np1,i2,1);
                k=k+1;
                nbU(i,k)= NPx(np2,i2,1);
        end
        for i2=1:2
                k=k+1;
                nbU(i,k)= NPx(np1,i2,2);
                k=k+1;
                nbU(i,k)= NPx(np2,i2,2);
        end
        for i2=1:2
            if NPx(np1,i2,3) ~= i
                k=k+1;
                nbU(i,k)= NPx(np1,i2,3);end
            if NPx(np2,i2,3) ~= i
                k=k+1;
                nbU(i,k)= NPx(np2,i2,3);end
        end
end
...
EE=-10;%-4.5*4186.798188/NA; %in kcal/mol
H=iniN(:,1);V=iniN(:,2);U=iniN(:,3);%use the initial set ratio p0
clear nH nV nU nTot ET ET0 ET1 ET2
eh=0;ev=0;eu=0;
for i=1:100
    if H(i,1)~=0
    ...
    eh=eh+(E1+0.5*E2*(abs(H(h1,1))+abs(H(h2,1))+ ...
        ...
        abs(U(u1,1))+abs(U(u2,1))+abs(U(u3,1))+abs(U(u4,1))))*abs(H(i,1));
    end
    ...
    ev=ev+(E1+0.5*E2*(abs(V(v1,1))+abs(V(v2,1))+ ...
        ...
        abs(U(u1,1))+abs(U(u2,1))+abs(U(u3,1))+abs(U(u4,1))))*abs(V(i,1));
    end
    ...
    eu=eu+(E1+0.5*E2*(abs(U(u1,1))+abs(U(u2,1))+ ...
        abs(H(h1,1))+abs(H(h2,1))+abs(H(h3,1))+abs(H(h4,1))+ ...
        abs(V(v1,1))+abs(V(v2,1))+abs(V(v3,1))+abs(V(v4,1))))*abs(U(i,1));
    end
end
et=eh+ev+eu;
...
for i=1:100
    ...
    eh=eh+(E1+0.5*E2*(abs(H(h1,itl+1))+abs(H(h2,itl+1))+ ...
        ...
        abs(U(u1,itl+1))+abs(U(u2,itl+1))+abs(U(u3,itl+1))+abs(U(u4,itl+1))))*abs(H(i,itl+1));
    end
    if V(i,itl+1)~=0
    h1=nbV(i,1);h2=nbV(i,2);h3=nbV(i,3);h4=nbV(i,4);
    v1=nbV(i,5);v2=nbV(i,6);
    ...
    ev=ev+(E1+0.5*E2*(abs(V(v1,itl+1))+abs(V(v2,itl+1))+ ...
        ...
        abs(U(u1,itl+1))+abs(U(u2,itl+1))+abs(U(u3,itl+1))+abs(U(u4,itl+1))))*abs(V(i,itl+1));
    end
    if U(i,itl+1)~=0
    h1=nbU(i,1);h2=nbU(i,2);h3=nbU(i,3);h4=nbU(i,4);
    v1=nbU(i,5);v2=nbU(i,6);v3=nbU(i,7);v4=nbU(i,8);
    u1=nbU(i,9);u2=nbU(i,10);
    eu=eu+(E1+0.5*E2*(abs(U(u1,itl+1))+abs(U(u2,itl+1))+ ...
        abs(H(h1,itl+1))+abs(H(h2,itl+1))+abs(H(h3,itl+1))+abs(H(h4,itl+1))+ ...
        abs(V(v1,itl+1))+abs(V(v2,itl+1))+abs(V(v3,itl+1))+abs(V(v4,itl+1))))*abs(U(i,itl+1));
    end
end
...
ET2(it,itl)=eh+ev+eu;
    ...
    if delE2 <=0
       U(j2,itl+1)=U(j2,itl+1);ET(it,itl+1)=ET2(it,itl);
    else if rand(1) <= exp((-delE2)/kBT(it,1))
            U(j2,itl+1)=U(j2,itl+1);ET(it,itl+1)=ET2(it,itl);
        else
            U(j2,itl+1)=U(j2,itl);ET(it,itl+1)=ET(it,itl+1);
        end
    end
...
nV(it,1)=sum(abs(V(:,1)));
nU(it,1)=sum(abs(U(:,1)));
nTot(it,1)=nH(it,1)+nV(it,1)+nU(it,1);
H_T(:,it)=H(:,itl+1);
V_T(:,it)=V(:,itl+1);
U_T(:,it)=U(:,itl+1);
nH(it,itl+1)=sum(abs(H(:,itl+1)));
nV(it,itl+1)=sum(abs(V(:,itl+1)));
nU(it,itl+1)=sum(abs(U(:,itl+1)));
nTot(it,itl+1)=nH(it,itl+1)+nV(it,itl+1)+nU(it,itl+1);
end
meanHVU(it,1)=mean(nH(it,itl-nave:itl+1));
meanHVU(it,2)=mean(nV(it,itl-nave:itl+1));
meanHVU(it,3)=mean(nU(it,itl-nave:itl+1));
meanHVU(it,4)=mean(nTot(it,itl-nave:itl+1));
end
subplot(2,2,3)
plot(step,nTot); ylim([0,100]);
legend(['T=10K:H=',num2str(meanHVU(1,1),'%0.1f'),',V=',num2str(meanHVU(1,2),'%0.1f'),', U=',num2str(meanHVU(1,3),'%0.1f'),],...
['T=100K'],['T=200K'],['T=300K'],['T=400K'],['T=500K'],['T=600K'],['T=700K'],...
['T=800K'],...
['T=1200K'],...
['T=1600K'],...
['T=2000K'],...
...
% plot map to new network figure at 2000K last step
Tii=12;%2000K
for i = 1:10
    for j=1:10
        ...
        ai=(11-i)*2;aj=j*2-1;
        if Ht(i,j)==1
            A(ai,aj,1)=0;A(ai,aj,2)=120;A(ai,aj,3)=255;%Horizontal+1 light
        else if Ht(i,j)==-1
                A(ai,aj,1)=0;A(ai,aj,2)=0;A(ai,aj,3)=255;%Horizontal-1 dark
            else
        ...
        bi=(11-j)*2-1;bj=i*2;
        if Vt(j,i)==1
            A(bi,bj,1)=200;A(bi,bj,2)=200;A(bi,bj,3)=200;% Vertical+1 light
        else if Vt(j,i)== -1
                A(bi,bj,1)=120;A(bi,bj,2)=120;A(bi,bj,3)=120;%Vertical-1 dark
            else 
...
xlim([0,41]);ylim([0,41]);
title(['T=2000K:H=',num2str(nH1+nH2),',V=',num2str(nV1+nV2),',U=',num2str(nU1+nU2)]); 
...
for i=1:100
    ...
    ev=ev+(E1+0.5*E2*(abs(V_E(v1,itl+1))+abs(V_E(v2,itl+1))+ ...
        ...
        abs(U_E(u1,itl+1))+abs(U_E(u2,itl+1))+abs(U_E(u3,itl+1))+abs(U_E(u4,itl+1))))*abs(V_E(i,itl+1))+EE*V_E(i,itl+1)*cos(pi()/3);
    end
    if U_E(i,itl+1)~=0
    h1=nbU(i,1);h2=nbU(i,2);h3=nbU(i,3);h4=nbU(i,4);
    v1=nbU(i,5);v2=nbU(i,6);v3=nbU(i,7);v4=nbU(i,8);
    ...
    eu=eu+(E1+0.5*E2*(abs(U_E(u1,itl+1))+abs(U_E(u2,itl+1))+ ...
        abs(H_E(h1,itl+1))+abs(H_E(h2,itl+1))+abs(H_E(h3,itl+1))+abs(H_E(h4,itl+1))+ ...
        abs(V_E(v1,itl+1))+abs(V_E(v2,itl+1))+abs(V_E(v3,itl+1))+abs(V_E(v4,itl+1))))*abs(U_E(i,itl+1))+EE*U_E(i,itl+1)*cos(-pi()/3);
    end
end
et=eh+ev+eu;
...
xlim([0,41]);ylim([0,41]);
title(['T=100K E-field:H=',num2str(nH1+nH2),',V=',num2str(nV1+nV2),',U=',num2str(nU1+nU2)]); 

% map to new network
% 2D hexagonal pattern 800K
ti=1;tj=1;tk=1;
nH1=0;nH2=0;nV1=0;nV2=0;nU1=0;nU2=0;
    Tii=4;%300K
    l=20;
...
xlim([0,41]);ylim([0,41]);
title(['T=300K E-field:H=',num2str(nH1+nH2),',V=',num2str(nV1+nV2),',U=',num2str(nU1+nU2)]); 

% map to new network
% 2D hexagonal pattern 1600K
ti=1;tj=1;tk=1;
nH1=0;nH2=0;nV1=0;nV2=0;nU1=0;nU2=0;
    Tii=5;%400K
    l=20;
...
xlim([0,41]);ylim([0,41]);
title(['T=400K E-field:H=',num2str(nH1+nH2),',V=',num2str(nV1+nV2),',U=',num2str(nU1+nU2)]); 

% map to new network
% 2D hexagonal pattern 2000K
ti=1;tj=1;tk=1;
nH1=0;nH2=0;nV1=0;nV2=0;nU1=0;nU2=0;
    Tii=6;%500K
    l=20;
...
xlim([0,41]);ylim([0,41]);
title(['T=500K E-field:H=',num2str(nH1+nH2),',V=',num2str(nV1+nV2),',U=',num2str(nU1+nU2)]); 
%% MFT approximation
clear result
clc
del=0.001;
...
for i = 1:lm
    ...
    ZE(j,i)=1+exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2+EE))+exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2-EE))+...
        exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2+EE*cos(pi()/3)))+exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2-EE*cos(pi()/3)))+...
        exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2+EE*cos(-pi()/3)))+ exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2-EE*cos(-pi()/3)))+...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE+EE*cos(pi()/3))) + exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE-EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE+EE*cos(pi()/3))) + exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE-EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE+EE*cos(-pi()/3))) + exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE-EE*cos(-pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE+EE*cos(-pi()/3))) + exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE-EE*cos(-pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE*cos(-pi()/3)+EE*cos(pi()/3))) + exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE*cos(-pi()/3)-EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE*cos(-pi()/3)+EE*cos(pi()/3))) + exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE*cos(-pi()/3)-EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2+EE*cos(-pi()/3)+EE*cos(pi()/3)+EE)) + exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2+EE*cos(-pi()/3)-EE*cos(pi()/3)+EE)) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2-EE*cos(-pi()/3)+EE*cos(pi()/3)+EE)) + exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2-EE*cos(-pi()/3)-EE*cos(pi()/3)+EE)) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2+EE*cos(-pi()/3)+EE*cos(pi()/3)-EE)) + exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2+EE*cos(-pi()/3)-EE*cos(pi()/3)-EE)) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2-EE*cos(-pi()/3)+EE*cos(pi()/3)-EE)) + exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2-EE*cos(-pi()/3)-EE*cos(pi()/3)-EE));

    mx1(j,i)=(exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2+EE))+ ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE+EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE-EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE+EE*cos(-pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE-EE*cos(-pi()/3))) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2+EE*cos(-pi()/3)+EE*cos(pi()/3)+EE)) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2+EE*cos(-pi()/3)-EE*cos(pi()/3)+EE)) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2-EE*cos(-pi()/3)+EE*cos(pi()/3)+EE)) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2-EE*cos(-pi()/3)-EE*cos(pi()/3)+EE)))/ZE(j,i);

    mx2(j,i)=(exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2-EE))+ ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE+EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE-EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE+EE*cos(-pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE-EE*cos(-pi()/3))) + ...
    ...
    my1(j,i)=(exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2+EE*cos(pi()/3)))+...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE+EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE+EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE*cos(-pi()/3)+EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE*cos(-pi()/3)+EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2+EE*cos(-pi()/3)+EE*cos(pi()/3)+EE)) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2-EE*cos(-pi()/3)+EE*cos(pi()/3)+EE)) + ...
        exp(-1/(kB*T)*(3*E1+(6*mH(1,i)+12*mV(1,i))*E2+EE*cos(-pi()/3)+EE*cos(pi()/3)-EE)) + ...
    ...
    my2(j,i)=(exp(-1/(kB*T)*(E1+(2*mH(1,i)+4*mV(1,i))*E2-EE*cos(pi()/3)))+...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE-EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE-EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2+EE*cos(-pi()/3)-EE*cos(pi()/3))) + ...
        exp(-1/(kB*T)*(2*E1+(4*mH(1,i)+8*mV(1,i))*E2-EE*cos(-pi()/3)-EE*cos(pi()/3))) + ...
...
axes1 = axes('Parent',figure1);
hold(axes1,'on');
plotMDT1 = plot([300 800	1200	1600	2000	2500], [1.8	7.0	39.8	64.5	79.4	85.4],'p-k','MarkerSize',18,'Parent',axes1);
plotMDT2 = plot([300 800	1200	1600	2000	2500], [0.0	2.0	11.7	22.2	29.7	27.0],'p-k','MarkerSize',2,'Parent',axes1);
plotMDE1 = plot ([10 100 200 300 400 500], [1.0	8.1	11.9	15.2	27.4	48.2],'p', 'Color',[1 0 1],'MarkerSize',18,'Parent',axes1);
plotMDE2 = plot ([10 100 200 300 400 500], [1.0	6.1	10.2	12.2	25.3	46.1],'p', 'Color',[1 0 1],'MarkerSize',2,'Parent',axes1);
plotMFT1 = plot(Ti,result(:,2)*100,'o','MarkerSize',18,'Parent',axes1);
plotMFT2 = plot(Ti,result(:,1)*100,'o','MarkerSize',2,'Parent',axes1);
plotMFE1 = plot(Ti,result(:,4)*100,'o','MarkerSize',18,'Parent',axes1);
plotMFE2 = plot(Ti,result(:,3)*100,'o','MarkerSize',2,'Parent',axes1);
plotMCT1 = plot([10,100,200,300,400,500,600,700,800,1200,1600,2000,2500], meanHVU(:,4),'s','MarkerSize',18,'Parent',axes1);
plotMCT2 = plot([10,100,200,300,400,500,600,700,800,1200,1600,2000,2500], meanHVU(:,1),'s','MarkerSize',2,'Parent',axes1);
...
%set(plot4(3),'DisplayName','MC, No E-field, U','LineWidth',1.5,'LineStyle','-.','Color',[0 1 1]);
%set(plot4(4),'DisplayName','MC, No E-field, Tot','LineWidth',1.5,'LineStyle','-','Color',[0 1 1]);
...
%set(plot5(3),'DisplayName','MC, E-field, U','LineWidth',1.5,'LineStyle','-.','Color',[1 0 0.7]);
%set(plot5(4),'DisplayName','MC, E-field, Tot','LineWidth',1.5,'LineStyle','-','Color',[1 0 0.7]);
box(axes1,'on');
hold(axes1,'off');
...
title(['2D hexagonal network: Ising model MC & MFT vs DPD']); 
result*100;
